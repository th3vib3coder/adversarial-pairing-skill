...
const VALID_PROVENANCE_KIND = new Set([
  'audit-finding', 'codebase-file', 'codebase-directory', 'protocol',
...
function validateFile(filePath) {
  ...
  if (!fields.provenance) {
    ...
    for (let idx = 0; idx < fields.provenance.length; idx++) {
      ...
      } else if (!VALID_PROVENANCE_KIND.has(item.kind)) {
        issues.push({
          file: filePath,
          field: 'provenance',
          problem: `provenance[${idx}] invalid kind "${item.kind}"; must be one of: ${[...VALID_PROVENANCE_KIND].join(', ')}`,
        });
      }
      if (!item.ref) {
