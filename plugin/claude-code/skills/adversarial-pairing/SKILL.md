...
These slash commands invoke the skill's tooling within a Claude Code session.

| Command | Purpose |
|---------|---------|
| `/hat-1-stop` | Generate a structured HAT 1 STOP report template with pre-survey evidence slots |
| `/adversarial-review` | Enter reviewer mode; run the six closure signals checklist against the current diff |
| `/dual-commit` | Step through the cross-repo dual-commit playbook interactively |
| `/init-pairing` | Bootstrap a new session: prompt for role declarations and pre-flight CI verification |
| `/lint-discipline` | Run wiki linter + registry check + ledger row validation in sequence |

**Hooks** (configured in `.claude/settings.json`):

- `ledger-mantra-check.js` — **advisory** hook; fires on every commit attempt and warns if
  the ledger row for the current seq is still in `pending` state.
- `pending-flip-guard.js` — **blocking** hook; prevents a commit from landing if the ledger
  row has not been flipped from `pending` to `OK` and reviewer ACCEPT is not on record.
