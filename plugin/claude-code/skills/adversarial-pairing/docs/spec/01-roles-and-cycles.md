...
soft checkpoint; it is a hard gate at which the reviewer issues a formal ACCEPT or BLOCK decision
and the operator authorizes the next phase either directly or through a valid standing
authorization. No seq is considered closed until HAT 3 ACCEPT is
...
- **HAT collapse**: the session skips HAT 1 entirely (no STOP report, no pre-survey, no RED plan)
  ...
  direct or standing operator authorization record between them.
...
- **Gate drift under time pressure**: operator approval is neither an explicit GO nor a scoped,
  attributable standing authorization, or its consumption is missing from the ledger; audit trail
  breaks.
...
In a phased delivery, each seq follows the pattern: HAT 1 STOP → reviewer ACCEPT/REDIRECT →
operator authorization → HAT 2 GREEN → HAT 3 closure → reviewer `ACCEPT-TO-FLIP` → operator
authorization → flip/stage → reviewer final HAT 3 `ACCEPT` → operator authorization → CI watch →
seq closed. Authorization is either a direct GO token or a recorded consumption of a valid
standing authorization. The implementer and reviewer roles are held by distinct agent instances.
...
- **HAT 1 — STOP**: The implementer produces a STOP report containing: (a) pre-flight CI
  ...
  reviewer ACCEPT or REDIRECT → if ACCEPT, operator authorization triggers HAT 2; if REDIRECT,
  implementer revises and re-submits HAT 1. No code is touched before a direct GO or a recorded
  standing-authorization consumption.
...
- **HAT 3 — Closure**: The implementer produces closure artifacts: full verification pass (tests,
  validate, lint, sync), a pending feature-ledger row, wiki sync (Tier A/B/C), and coherent save
  targets. The reviewer inspects the pre-flip working-tree diff and every authorized untracked
  file; the index may be empty and an empty cached diff is not missing evidence. The reviewer may
  then issue provisional `ACCEPT-TO-FLIP`. Direct `GO-TO-FLIP` or its valid standing-authorization
  consumption authorizes only the current-row pending→OK transition and staging. The reviewer
  reruns the six signals on the complete cached patch and may issue final HAT 3 `ACCEPT`; direct
  `GO-COMMIT` or its valid standing-authorization consumption authorizes commit/push.
...
The ordering is non-negotiable. Attempting HAT 2 before operator authorization on HAT 1 means the design
may change mid-implementation (wasted work, inconsistent artifacts). Attempting HAT 3 before HAT 2
is complete means closure artifacts describe a non-green state. Flipping before
`ACCEPT-TO-FLIP`/flip authorization, or committing before final HAT 3 `ACCEPT` and commit
authorization, makes
the gate decorative rather than enforceable.
...
The operator either issues GO or the implementer records consumption of a previously granted,
in-scope standing authorization. Implementer writes failing tests, then implementation. Tests green, no regressions.
...
Reviewer inspects the working-tree diff and authorized untracked files, with an empty index allowed,
then issues `ACCEPT-TO-FLIP`; operator flip authorization is recorded; implementer flips the current R2 cell,
checks that exact structural cell, and stages the patch. Reviewer inspects the complete cached diff
and issues final HAT 3 `ACCEPT`; operator commit authorization is recorded. Implementer commits, pushes, and
watches CI.

---

#### §1.5.2.1 Direct gates and standing operator authorization

Direct-gate mode requires a fresh operator token at each transition. An operator who wants a
bounded outcome completed without repeated pauses may instead grant standing authorization once.
It must name the objective, repositories, branches, seq range, permitted transitions and external
effects, expiry, and exclusions. The implementer records a separate normalized consumption at
HAT 1, flip/stage, and commit/push; no new conversational prompt is required while scope remains
valid.

Standing authorization never permits implementer self-review, never replaces either HAT 3 review
surface, and never implies merge/tag/release, destructive actions, force-push, credentials, or
scope expansion. Ambiguity, revocation, expiry, reviewer BLOCK, or an out-of-scope recovery pauses
the cycle. The complete record format and pause rules are normative in
[Appendix E](appendices/E-standing-operator-authorization.md).
...
`READY: bootstrap structure complete; operator authorization required for first commit`. The tool
never commits. A later reviewed commit may consume standing authorization only when that effect
and target repository were named explicitly.
...
Implementer pushes after HAT 3 ACCEPT and recorded operator commit authorization. Opens CI dashboard, monitors the triggered
